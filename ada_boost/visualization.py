...
def plot_performance_comparison(svm_df, adaboost_df, timestamp, save_path='bin'):
    """
    绘制各模型性能比较图
    ...
    for model, df in zip(['svm', 'adaboost'], [svm_df, adaboost_df]):
        ...
        # 绘制准确率比较
        sns.barplot(x='Model', y='Accuracy', data=df, ax=axes[0])
        ...
        # 绘制F1值比较
        sns.barplot(x='Model', y='F1 Score', data=df, ax=axes[1])
        ...
        # 绘制训练时间比较
        sns.barplot(x='Model', y='Training Time (s)', data=df, ax=axes[2])
        axes[2].set_title('Model Training Time')
        axes[2].set_xticks(ticks_pos)
        axes[2].set_xticklabels(model_labels, rotation=45, ha='right')
        
        plt.tight_layout()
        plt.savefig(save_path + f'/{model}_model_performance_comparison_{timestamp}.png', dpi=300)
        plt.close()

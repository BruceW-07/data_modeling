...
def plot_performance_comparison(performance_df, save_path='model_performance_comparison.png'):
    """
    绘制各模型性能比较图
    ...
    # 绘制准确率比较
    sns.barplot(x='Model', y='Accuracy', data=performance_df, ax=axes[0], palette='viridis')
    axes[0].set_title('模型准确率比较')
    axes[0].set_ylim(0.8, 1.0)  # 设置y轴范围
    axes[0].set_xticklabels(axes[0].get_xticklabels(), rotation=45, ha='right')
    
    # 绘制F1值比较
    sns.barplot(x='Model', y='F1 Score', data=performance_df, ax=axes[1], palette='viridis')
    axes[1].set_title('模型F1值比较')
    axes[1].set_ylim(0.8, 1.0)  # 设置y轴范围
    axes[1].set_xticklabels(axes[1].get_xticklabels(), rotation=45, ha='right')
    
    # 绘制训练时间比较
    sns.barplot(x='Model', y='Training Time (s)', data=performance_df, ax=axes[2], palette='viridis')
    axes[2].set_title('模型训练时间比较')
